# 👋 Привет

Я -инженер и практик в области ИИ, автоматизации и цифровых ассистентов.  
Создаю рабочие системы на стыке **LLM, автоматизации и бизнес-логики**.


## 🧠 Чем я занимаюсь

- 🤖 AI-ассистенты (GPT / RAG / агентные сценарии)
- 🔗 Автоматизация через **n8n, Make, Telegram**
- 🧩 Структурированные данные (JSON, Google Sheets, API)
- 🛠️ MVP → production под реальные задачи
- 📊 Пайплайны: ввод → анализ → логика → результат


## 🛠️ Стек и инструменты

### AI / LLM
![OpenAI](https://img.shields.io/badge/OpenAI-LLM-black)
![RAG](https://img.shields.io/badge/RAG-Vector_DB-blue)
![Prompt Engineering](https://img.shields.io/badge/Prompt-Engineering-purple)

### Automation / Backend
![Python](https://img.shields.io/badge/Python-3.10+-blue)
![n8n](https://img.shields.io/badge/n8n-workflows-orange)
![Make](https://img.shields.io/badge/Make-automation-purple)
![Docker](https://img.shields.io/badge/Docker-containerization-blue)

### Integrations
![Telegram](https://img.shields.io/badge/Telegram-Bots-2CA5E0)
![Google Sheets](https://img.shields.io/badge/Google-Sheets-green)
![REST API](https://img.shields.io/badge/API-REST-lightgrey)


## 📌 Pinned-проекты

### 🤖 AI Assistant Platform
**Персональный AI-ассистент**
- задачи, дедлайны, напоминания
- Telegram + n8n + OpenAI
- Google Sheets как storage  
🔗 https://github.com/USERNAME/ai-assistant


### 🧠 HR Matching Agent
**AI-матчинг кандидатов и вакансий**
- скоринг, soft-matching
- structured output
- готово для HR-MVP  
🔗 https://github.com/USERNAME/hr-matching-agent


### 🌱 RAG Knowledge Base
**База знаний + RAG**
- JSONL-карточки
- векторная БД
- региональная логика (Россия)  
🔗 https://github.com/USERNAME/rag-knowledge-base


### 📄 AI for Business
**Генерация КП, презентаций, инструкций**
- бизнес-документы
- шаблоны и пайплайны
- адаптация под клиента  
🔗 https://github.com/USERNAME/ai-business-tools


## 📊 GitHub статистика

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=julipel&show_icons=true&hide_title=true&count_private=true&theme=default)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=julipel&layout=compact&theme=default)

![Streak](https://github-readme-streak-stats.herokuapp.com/?user=julipel)


## 🎯 Подход

- Системность вместо магии  
- AI как инструмент, а не культ  
- Масштабируемость и поддерживаемость  
- Решения, которые живут после MVP


## 📬 Контакты

- GitHub: https://github.com/USERNAME  
- LinkedIn: https://linkedin.com/in/USERNAME  
- Telegram: @USERNAME  


⭐ Если проекты полезны — ставьте звёзды и подписывайтесь
